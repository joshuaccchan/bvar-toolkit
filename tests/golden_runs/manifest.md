...
| `chan2020_jbes_kronecker` | `main_BVAR.m` | min | Full-sample, 8 models, 30000+5000 draws, one model per run via the in-script selector. Legacy `randn('seed',...)` syntax verified working on R2025b - no patch needed. `addpath('./realtime_forecasts')` creates path-order shadowing between root and realtime copies of sample_h/llike_CSV_MA - record which copy resolved when adjudicating ML values. CAPTURED 2026-09-01: model 8 (BVAR-CSV-t-MA, as-shipped default) log-ML -8468.4 in ~92 s - this value flows through the known stale-Sig bug in ml_BVAR_CSV_t_MA and is the bugcompat reference; models 1-7 captured via selector patches (see their golden logs). |
