...
%   bvt.ml.lniwpdf      = legacy lniwpdf.m
%   bvt.ml.linvgammpdf  = legacy linvgammpdf.m
%   bvt.ml.llike_ma     = legacy llike_MA.m (root)
%   bvt.ml.llike_csv_ma = legacy llike_CSV_MA.m (package ROOT copy)
...
% --- core values ---
c_lniw = bvt.ml.lniwpdf(A, Sig, A0, iVA0, nu0, S0);
yv = [.5 1.2 3]; av = [2 3.5 4]; bv = [.5 .8 2];
c_ligp = bvt.ml.linvgammpdf(yv, av, bv);
c_lma = bvt.ml.llike_ma(psi, U, Sig);
c_lcsvma = bvt.ml.llike_csv_ma(psi, U, Sig, h);
...
assert(isequal(lniwpdf(A, Sig, A0, iVA0, nu0, S0), c_lniw), ...
    'bvt.ml.lniwpdf differs from legacy lniwpdf');
assert(isequal(linvgammpdf(yv, av, bv), c_ligp), ...
    'bvt.ml.linvgammpdf differs from legacy linvgammpdf');
assert(isequal(llike_MA(psi, U, Sig), c_lma), ...
    'bvt.ml.llike_ma differs from legacy llike_MA');
l_root = llike_CSV_MA(psi, U, Sig, h);
assert(isequal(l_root, c_lcsvma), ...
    'bvt.ml.llike_csv_ma differs from the legacy ROOT llike_CSV_MA');
clear c1
