function test_igrnd
% bvt.util.igrnd must draw identically to the inline 1/gamrnd(...) expression
% the legacy samplers use, under the same seed.

nu = 5; S = 0.04;

rng(11, 'twister');
x_core = bvt.util.igrnd(nu, S);
rng(11, 'twister');
...
rng(13, 'twister');
s_core = bvt.util.igrnd(nuh0 + T/2, Sh0 + sum(e.^2)/2);
rng(13, 'twister');
...
rng(15, 'twister');
nu_m = 8; S_m = 3;
draws = bvt.util.igrnd(nu_m*ones(2e5,1), S_m*ones(2e5,1));
...
try
    ...
    assert(strcmp(err.identifier, 'bvt:util:igrnd:badParam'), 'igrnd: wrong error id');
end
end
