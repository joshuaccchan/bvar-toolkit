% bvt.ml.kron_bvar_csv_ma - log marginal likelihood of the BVAR-CSV-MA model
% of Chan (2020, JBES): integrated likelihood by importance sampling
% (bvt.ml.intlike_csv_ma) at the posterior means, prior ordinate, posterior
...
%
%   [ML, out] = bvt.ml.kron_bvar_csv_ma(shortY, X, pri, est, ...)
%
...
for iv = 1:2:numel(varargin)
    switch lower(varargin{iv})
        ...
        otherwise, error('bvt:ml:kron_bvar_csv_ma:badOption', ...
...
llike = bvt.ml.intlike_csv_ma(shortY,X,A_mean,Sig_mean,theta_mean(1),...
...
for isim = 1:nsims
    ...
    lden_sigh2 = bvt.ml.linvgammpdf(theta_mean(3),nuh0+T/2,Sh0 + sum(eh.^2)/2);
...
for isim = 1:nsims2
    ...
    s2 = sum(tmp.^2,2);
    s2(1) = s2(1)/(1+psi^2);
    h = bvt.sv.csv_armh(s2,rho,sigh2,h,n);              % legacy line 67: root sample_h
    ...
    % sample psi
    lp_psi = @(x) bvt.ml.llike_csv_ma(x,U,Sig_mean,h) + lpri_psi(x);
