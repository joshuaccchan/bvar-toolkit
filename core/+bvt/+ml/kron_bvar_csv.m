% bvt.ml.kron_bvar_csv - log marginal likelihood of the BVAR-CSV model of
% Chan (2020, JBES): integrated likelihood by importance sampling
% (bvt.ml.intlike_csv, h integrated out) at the posterior means, prior
...
% flag needed. The reduced run's h step is bvt.sv.csv_armh with NR start
% h_mean and a first-sweep forced accept.
%
%   [ML, out] = bvt.ml.kron_bvar_csv(shortY, X, pri, est, ...)
%
...
for iv = 1:2:numel(varargin)
    switch lower(varargin{iv})
        ...
        otherwise, error('bvt:ml:kron_bvar_csv:badOption', ...
...
llike = bvt.ml.intlike_csv(shortY,X,A_mean,Sig_mean,theta_mean(1),theta_mean(2),R);
...
lpri = -.5*log(2*pi*Vrho) + log(c_rho) -.5*(theta_mean(1)-rho0)^2/Vrho ...
    ...
    + bvt.ml.lniwpdf(A_mean,Sig_mean,A0,sparse(1:k,1:k,1./VA0),nu0,S0);
...
for isim = 1:nsims
    ...
    lden_ASig = bvt.ml.lniwpdf(A_mean,Sig_mean,Ahat,KA,nu0+T,Shat);

        % compute the conditional density of sigh2
    eh = [h(1)*sqrt(1-rho^2);  h(2:end)-rho*h(1:end-1)];
    lden_sigh2 = bvt.ml.linvgammpdf(theta_mean(2),nuh0+T/2,Sh0 + sum(eh.^2)/2);
...
for isim = 1:nsims
        ...
        % accept on the reduced run's first sweep -> bvt.sv.csv_armh
    h = bvt.sv.csv_armh(s2,rho,sigh2,h,n,isim==1,h_mean);
