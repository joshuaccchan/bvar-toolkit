% bvt.ml.kron_bvar_ma - log marginal likelihood of the BVAR-MA model by
% Chib's method. Deterministic given the stores: consumes no rng.
%
%   [ML, out] = bvt.ml.kron_bvar_ma(shortY, X, pri, est, ...)
%
...
% body verbatim (lniwpdf/llike_MA -> the bvt.ml equivalents; the lpri_psi
...
for iv = 1:2:numel(varargin)
    switch lower(varargin{iv})
        ...
        otherwise, error('bvt:ml:kron_bvar_ma:badOption', ...
...
lpri = bvt.ml.lniwpdf(A_mean,Sig_mean,A0,sparse(1:k,1:k,1./VA0),nu0,S0) ...
...
U = shortY - X*A_mean;
lp_psi = @(x) bvt.ml.llike_ma(x,U,Sig_mean) + lpri_psi(x);
