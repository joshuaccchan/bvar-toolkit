% bvt.ml.kron_bvar_csv_t_ma - log marginal likelihood of the BVAR-CSV-t-MA
...
%
%   [ML, out] = bvt.ml.kron_bvar_csv_t_ma(shortY, X, pri, est, ...)
%
...
for iv = 1:2:numel(varargin)
    switch lower(varargin{iv})
        ...
        otherwise, error('bvt:ml:kron_bvar_csv_t_ma:badOption', ...
...
nu_mean = theta_mean(4);
    ...
    % [lines 20-28]
llike = bvt.ml.intlike_csv_t_ma(shortY,X,A_mean,Sig_mean,psi_mean,rho_mean,...
...
for isim = 1:nsims2
    ...
    h = bvt.sv.csv_armh(s2_h,rho,sigh2,h,n);            % legacy line 87: root sample_h
    ...
    lp_psi = @(x) bvt.ml.llike_csv_ma(x,U_psi,Sig_psi,h) + lpri_psi(x);
