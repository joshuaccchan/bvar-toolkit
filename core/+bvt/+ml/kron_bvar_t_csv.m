% bvt.ml.kron_bvar_t_csv - log marginal likelihood of the BVAR-t-CSV model of
% Chan (2020, JBES): integrated likelihood by importance sampling
% (bvt.ml.intlike_t_csv, lam analytic and h by IS) at the posterior means,
...
%
%   [ML, out] = bvt.ml.kron_bvar_t_csv(shortY, X, pri, est, ...)
%
...
for iv = 1:2:numel(varargin)
    switch lower(varargin{iv})
        ...
        otherwise, error('bvt:ml:kron_bvar_t_csv:badOption', ...
...
llike = bvt.ml.intlike_t_csv(shortY,X,A_mean,Sig_mean,rho_mean,sigh2_mean,...
    nu_mean,R);
...
for isim = 1:nsims
    ...
    lden_sigh2 = bvt.ml.linvgammpdf(sigh2_mean,nuh0+T/2,Sh0 + sum(eh.^2)/2);
...
for isim = 1:nsims
    s2 = sum(tmp.^2,2)./lam;
    h = bvt.sv.csv_armh(s2,rho,sigh2,h,n);              % legacy line 70: root sample_h
