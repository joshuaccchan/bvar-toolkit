% bvt.samplers.eq_svar_oi - equation-by-equation Gaussian draw of the VAR
...
% Vbeta output of bvt.priors.vtheta on the same inputs).
%
...
% the unqualified vec calls now bvt.util.vec (code-identical to the legacy
...
k = size(X,2);
n = size(Y,2);
Lambda = bvt.util.vec(exp(h/2));
for ii=1:n
    A(:,ii) = 0;
    yi = bvt.util.vec((Y-X*A)*B0')./Lambda;
