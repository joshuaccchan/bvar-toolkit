% bvt.util.igrnd - draws from the inverse-gamma distribution in the (nu, S)
...
% and residuals e - is
%
%       sig2 = bvt.util.igrnd(nu0 + T/2, S0 + sum(e.^2)/2);
%
...
if nargin < 2
    error('bvt:util:igrnd:nargin', 'igrnd needs both nu and S');
end
if any(nu(:) <= 0) || any(S(:) <= 0)
    error('bvt:util:igrnd:badParam', 'nu and S must be positive');
end
x = 1./gamrnd(nu, 1./S);
end
