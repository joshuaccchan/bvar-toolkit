% bvt.structural.b0_row_sampler - one full row-wise pass of the posterior draw
...
% two-component approximation bvt.util.anormrnd and the remaining n-1
% coordinates as zetaj_hat + randn/sqrt(T), maps back, and fixes the sign of
% the ii-th element to be positive.
%
%   B0 = bvt.structural.b0_row_sampler(U, h, B0, B00, VB0)
%
...
% unqualified anormrnd call now bvt.util.anormrnd (code-identical to the legacy
...
for ii=1:n
    ...
    for jj=1:n
        if jj==1
            ...
            V = [v1 null(v1')];
            zetaj_hat = mui'*(Ci*v1);
            zeta(1) = bvt.util.anormrnd(zetaj_hat,1/T);
        else
