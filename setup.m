...
if isempty(ver('optim'))
    warning('bvar:setup:noOptim', ...
        ['The Optimization Toolbox is not available. The marginal-likelihood ' ...
         'functions for the MA-error models (bvar.ml.kron_bvar_ma, ' ...
         'kron_bvar_t_ma, kron_bvar_csv_ma, kron_bvar_csv_t_ma) call fminunc, ' ...
         'as do the chan2020_springer_largebvar and cjz2019_ad_opthyper drivers.']);
end
if isempty(ver('ident'))
    warning('bvar:setup:noIdent', ...
        ['The System Identification Toolbox is not available. Only one ' ...
         'replication driver needs it: cjz2018_ad_var''s main_AD_VAR.m calls ' ...
         'ar() when building its Minnesota prior.']);
end
