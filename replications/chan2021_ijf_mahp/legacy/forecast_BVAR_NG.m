% This function computes the forecasts from a large Bayesian VAR with the 
% normal-gamma prior
%
% See:
...
for isim = 1:nsim + burnin
        % sample alp and beta         
    [Valp,Vbeta] = getVtheta(idx_kappa1,idx_kappa2,[kappa,kappa,kappa3,kappa4],Psi,sig2);
    ...
    for ij=1:n
        ...
        % sample kappa
    tmpc1 = sum(beta(idx_kappa1).^2./(2*psi_kappa1));
    tmpc2 = sum(beta(idx_kappa2).^2./(2*psi_kappa2));
    kappa = gigrnd(c01(1)-n^2*p/2,2*c01(2),tmpc1+tmpc2,1);     
    
        % sample psi
    tmpv1 = beta(idx_kappa1).^2/(2*kappa);
    tmpv2 = beta(idx_kappa2).^2/(2*kappa);    
    for ik=1:1:n*p  % set lower bound 1e-16 to avoid arithmetic underflow
        psi_kappa1(ik) = max(gigrnd(nu_psi-1/2,nu_psi,tmpv1(ik),1),1e-16);
    end
    for il=1:1:(n-1)*n*p
        psi_kappa2(il) = max(gigrnd(nu_psi-1/2,nu_psi,tmpv2(il),1),1e-16);
    end    
