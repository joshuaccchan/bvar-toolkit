...
while nonEmptyDraw < opt.phiDraws
    ...
    % Use algorithm from GKV20 to determine whether identified set is 
    % nonempty.
    [empty,restr.F,restr.S] = checkEmptyIS_GKV(restr,phi,opt);
    
    if empty == 0 % If identified set is nonempty
    
        nonEmptyDraw = nonEmptyDraw + 1;
                
        % Compute identified sets for impulse responses of interest using
