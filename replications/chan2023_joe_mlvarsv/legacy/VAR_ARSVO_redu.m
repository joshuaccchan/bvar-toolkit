% This script estimates the VAR-SVO model in reduced-form with stationary
...
ngrid = 31;    % # of grid points in the grid for o
o_grid = [1;linspace(2,20,ngrid)'];  % grid for o
...
for isim = 1:nsim + burnin
    ...
    for ii = 1:n        
        A(:,ii) = 0;
        Lambda = vec(sqrt_exph(:,ii:n).*repmat(o,1,n-ii+1));
    ...
    for tt=1:T
        ...
        % sample po
    tmp = sum(o>1);
    po = betarnd(Hyper.p0a + tmp, Hyper.p0b + T-tmp);
...
o_mean = mean(store_o)';
po_mean = mean(store_po);

    % compute the marginal likelihood
if cp_ml
    disp(['Computing the marginal likelihood of ' model_name '... ']);
    start_time = clock;
    [lml,lmlstd] = ml_var_arsvo_redu(X,Y,Y0,M,Hyper,flag_marg,store_h,...
        store_beta,store_hpara,store_kappa,store_o,store_po,o_grid,is_kappafixed,is_kappasym);
