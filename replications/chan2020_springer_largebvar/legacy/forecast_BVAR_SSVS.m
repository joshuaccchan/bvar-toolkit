% This script produces forecasts from a 20-variable BVAR with the SSVS prior
%
% See:
...
for isim = 1:nsims + burnin
           % sample beta    
    iVbeta = sparse(1:n*k,1:n*k,1./((1-gam).*V_Minn + gam*kappa1));
    ...
    Sig = iwishrnd(S0+Ut*Ut',nu0+Tt);    
        ...
        % sample gam
    prob0 = (1-q)*normpdf(beta,zeros(n*k,1),sqrt(V_Minn));
    prob1 = q*normpdf(beta,zeros(n*k,1),sqrt(kappa1)*ones(n*k,1));
    prob1 = prob1./(prob1+prob0);
    gam = (prob1>rand(n*k,1));
